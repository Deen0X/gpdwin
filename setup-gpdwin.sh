...
echo "🌐 [2/9] Instalando Chrome Remote Desktop..."
...
echo "🖥️ [3/9] Instalando gestor de pantallas (Arandr)..."
sudo apt install -y arandr

echo "🧩 [4/9] Creando script de gestión automática de pantallas..."
...
echo "🔁 [5/9] Automatizando gestión de pantallas con cron..."
(crontab -l 2>/dev/null; echo "* * * * * $HOME_DIR/monitor-watch.sh") | crontab -

echo "🔄 [6/9] Creando alias para rotar pantalla integrada..."
echo "alias rotate='xrandr --output DSI-1 --rotate right'" >> $HOME_DIR/.bashrc

echo "🎮 [7/9] Instalando RetroPie..."
